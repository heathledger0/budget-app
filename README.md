...
- **월별 리포트**: 선택한 달의 수입/지출 요약, 전월 대비 증감, 일별 수입·지출 추이, 그 달의 카테고리별 지출 비중(막대·파이, 섹션 → 카테고리 → 개별 항목까지 드릴다운). 보증금처럼 금액이 큰 1회성 항목(수입의 "보증금 환급", 저축/투자의 "보증금 납부" 카테고리)은 체크박스로 통계에서 제외 가능(기본 제외)
- **연간 리포트**: 카테고리별 연간 지출 비중(막대·파이 차트, 섹션 → 카테고리 → 개별 항목까지 드릴다운), 월별 요약 테이블, 연간 합계·월평균. 월별 리포트와 같은 1회성 큰 항목 제외 체크박스 제공
- **신용카드 트래커**: 카드를 쓸 때마다 기록하는 누적 사용 내역. 항목마다 카테고리(와 선택적으로 날짜)를 고르면 그 지출이 해당 카테고리 합계·예산 비교·리포트에도 자동으로 반영되어, 월별 입력에 같은 지출을 또 적을 필요가 없음(카테고리를 고르지 않으면 예전처럼 트래커에서만 누적)
...
- **앱 업데이트 알림**: 새 버전이 배포되면 화면 하단에 "새 버전이 있어요" 배너가 뜨고, 버튼 한 번으로 바로 최신 버전을 반영 (홈 화면에 설치한 PWA에서 캐시를 직접 지울 필요 없음)
...
기존에 Supabase 프로젝트를 만들어 쓰고 있었다면, 신용카드 트래커에서 카테고리와 날짜를 지정할 수 있으려면 [`supabase/schema.sql`](./supabase/schema.sql)을 SQL Editor에서 한 번 더 실행해 `card_entries` 테이블에 `category_id`, `day` 열을 추가해야 합니다.
