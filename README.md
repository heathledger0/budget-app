...
- **신용카드 트래커**: 카드를 쓸 때마다 기록하는 누적 사용 내역. 항목마다 카테고리를 고르면 그 지출이 해당 카테고리 합계·예산 비교·리포트에도 자동으로 반영되어, 월별 입력에 같은 지출을 또 적을 필요가 없음(카테고리를 고르지 않으면 예전처럼 트래커에서만 누적)
...
기존에 Supabase 프로젝트를 만들어 쓰고 있었다면, 신용카드 트래커에서 카테고리를 고를 수 있으려면 [`supabase/schema.sql`](./supabase/schema.sql)을 SQL Editor에서 한 번 더 실행해 `card_entries` 테이블에 `category_id` 열을 추가해야 합니다.
