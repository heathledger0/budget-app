...
- **월별 리포트**: 선택한 달의 수입/지출 요약, 전월 대비 증감, 일별 수입·지출 추이, 그 달의 카테고리별 지출 비중(막대·파이, 섹션 → 카테고리 → 개별 항목까지 드릴다운). 보증금처럼 금액이 큰 1회성 항목(수입의 "보증금 환급", 저축/투자의 "보증금" 카테고리)은 체크박스로 통계에서 제외 가능(기본 제외)
